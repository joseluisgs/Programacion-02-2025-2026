...
Los bucles permiten repetir un bloque de código varias veces.
  ...
  * **Bucles definidos (`for`)**: Se utilizan cuando se sabe de antemano cuántas veces se quiere repetir un bloque de código. Se componen de tres partes: inicialización, condición y actualización. `foreach` es una variante que recorre cada elemento de una colección.
  * **Recorriendo arrays con `for` y `foreach`**: Una aplicación común de los bucles es recorrer colecciones de datos, como arrays.
...
Los **parámetros** son las variables definidas en la declaración de una función o procedimiento. Los **argumentos** son los valores reales que se pasan al llamar a la función y se debe cumplir la **seguridad y estrictud de tipos**:

1.  **Estrictud de tipos:** Se exige el *casting* explícito para cualquier conversión entre tipos.
2.  **Excepción Segura:** La única excepción permitida es la conversión segura de un tipo base (`T`) a su versión anulable (`T?`), ya que es una conversión de contenedor sin riesgo de pérdida de datos.
3.  **Prohibición Inversa:** Se prohíbe el paso de un anulable (`T?`) a un tipo base (`T`) sin manejo explícito, forzando la verificación de `null` y evitando errores en tiempo de ejecución.
  


**Confirmo que la regla de tipos para DAW está registrada:**

> Para el lenguaje DAW, la regla es: los tipos de los argumentos pasados a una función o procedimiento deben **coincidir exactamente** con los tipos de los parámetros definidos. Si un tipo de menor rango necesita ser pasado a uno de mayor rango, **se debe usar *casting* explícito**, **salvo** en la conversión de un tipo base (`T`) a su tipo anulable (`T?`). [cite: 2025-10-07]

¿Te gustaría que desarrollemos el siguiente punto del temario, **9.3. Paso por Valor vs. Paso por Referencia**, o tienes alguna otra duda sobre funciones?
